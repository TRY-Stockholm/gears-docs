---
title: Capabilities
description: What Gears ships out of the box and what you'd add yourself — by capability rather than by package.
---
...
This page is the map. Cards are tagged **Integrated** when Gears ships a packaged provider and a guide, and **Not yet** when the capability is recognized but unaddressed today. Six live storefronts run on this stack, so anything marked Integrated is real production code, not a demo.
...
<CapabilityGrid>
  ...
  <CapabilityCard
    title="Tax"
    status="none"
    description="Calculated by Centra per market — no Gears layer needed. Clients configure tax rates inside the Centra dashboard, not in code."
  />
  <CapabilityCard
    title="Returns / RMA"
    status="none"
